...
DECLARE @visit_start_date_from datetime = '2018-01-01'
DECLARE @visit_start_date_until datetime = '2018-01-31 23:59:59.999'
DECLARE @hosptal_code varchar(50) = 'AHI'
DECLARE @other_language_rcd varchar(50) = 'EN'
DECLARE @local_language_rcd varchar(50) = 'EN'

DECLARE @visit AS TABLE(patient_visit_id guid PRIMARY KEY, patient_id guid, visit_type_rcd varchar(50), actual_visit_date_time datetime, arrival_mode_rcd varchar(50), patient_triage_history_id guid, patient_queue_supported_flag bit)
...
SELECT          pv.patient_visit_id
                ...
                , vtr.patient_queue_supported_flag
...
--OPTION (TABLE HINT (pv, INDEX (XIE6patient_visit)))
...
INNER JOIN      api_patient_view apv WITH (FORCESEEK) ON apv.patient_id = pv.patient_id
LEFT JOIN       address_label_view AS al ON al.address_id = apv.home_address_id

-- triage
LEFT JOIN      patient_triage_history_nl_view AS pth ON  pth.patient_triage_history_id = pv.patient_triage_history_id 
