DECLARE @year1 int
DECLARE @year int
set @year1 = year(DATEADD(Year,-2,GETDATE()))
...
select tempb.patient_id
	   ...
	   ,tempb.[Address Type]
	   ,tempb.Age
	   ,case when tempb.Age between 0 and 19 then '0-19'
			 when tempb.Age between 20 and 29 then '20-29'
			 when tempb.Age between 30 and 39 then '30-39'
			 when tempb.Age between 40 and 49 then '40-49'   
			 when tempb.Age >= 50 then '>50' end as [Age Group]
	   ,tempb.[Date of Birth]
	   ,tempb.Nationality
	   ,tempb.Race
	   ,tempb.[Ethnic Group]
	   ...
	   ,tempb.Country
	   ,case when tempb.Country is null or tempb.Country = 'Unknown' then 'Unknown'
			 when tempb.Country = 'Philippines' then 'Local'
	         when (tempb.Country is not null and tempb.Country <> 'Philippines') then 'International'
			 end as [Country Type]
	   ,tempb.[Home Address]
	   ,tempb.[Primary Service]
	   ,tempb.Packages
	   ,case when (tempb.Subregion is null and tempb.Region is null and tempb.Country <> 'Philippines') then 'International'
		     when (tempb.Subregion is null or tempb.Region is null) then 'Unspecified'
			 else tempb.[Cardinal Direction] end as [Cardinal Direction]
	   ,tempb.Subregion + ', ' + (case when tempb.Subregion = 'Aurora' then 'Region III (Central Luzon)' else tempb.Region end) + ', ' + tempb.residence_country_rcd as Location
...
FROM api_patient_visit_view api 
			...
			left join country_ref cref on apv.residence_country_rcd = cref.country_rcd
...
where 
	  --year(actual_visit_date_time) = 2017
   --   and month(actual_visit_date_time) between 1 and 12
...
where temp.[Address Type] in ('H1','N/A')
      and year(temp.[Visit Start]) between @year1 and @year
	  and temp.visit_type_rcd = 'V4'
	  --and temp.HN in ('00503473', '00373498', '00486768')
	  --and temp.HN = '00258739'
	  --and temp.residence_country_rcd is null
)as tempb
