...
select tempc.Residence
       ,count(tempc.Residence) as Count
...
select tempb.patient_id
	   ...
	   ,case when Country = 'Philippines' then 'Philippines'
			 when Country is null or tempb.Country <> 'Philippines' then 'Other countries'
			 end as Residence
...
select distinct apv.patient_id
	   ...
	   ,case when adnv.city_id is NOT NULL then  (SELECT name_l
		...
		,datediff(dd,apv.date_of_birth, '12/31/18') / 365 as Age
