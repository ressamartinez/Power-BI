...
select tempb.patient_id
	   ...
	   --,tempb.Subregion + ', ' + (case when tempb.Subregion = 'Aurora' then 'Region III (Central Luzon)' else tempb.Region end) + ', ' + tempb.home_address_country_rcd as Location
...
where year(tempb.[Visit Start]) = 2018
	  ...
	  -- CAST(CONVERT(VARCHAR(10),tempb.[Visit Start],101) as SMALLDATETIME) >= CAST(CONVERT(VARCHAR(10),@From,101) as SMALLDATETIME)
	  --and CAST(CONVERT(VARCHAR(10),tempb.[Visit Start],101) as SMALLDATETIME) <= CAST(CONVERT(VARCHAR(10),@To,101) as SMALLDATETIME)
